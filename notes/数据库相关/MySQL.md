...
mysql   四大索引 ： B+树索引 ,哈希索引，全表索引，空间数据索引
...
平衡树查找操作的时间复杂度和树高 h 相关，O(h)=O(logdN)，（以d为底的log N）其中 d 为每个节点的出度。
...
## MySQL 索引   （哪几大类索引 4大）
 ...
 四大索引 ： B+树索引 ,哈希索引，全表索引，空间数据索引

索引是在存储引擎层实现的，而不是在服务器层实现的，所以不同存储引擎具有不同的索引类型和实现。

### 1. B+Tree 索引  （为什么MySQL用B+Tree索引）

是大多数 MySQL 存储引擎的默认索引类型。

因为有B+Tree不再需要进行全表扫描，只需要对树进行搜索即可，所以查找速度快很多。
...
（B+Tree 索引 分类）InnoDB 的 B+Tree 索引分为主索引和辅助索引。主索引的叶子节点 data 域记录着完整的数据记录，这种索引方式被称为聚簇索引。因为无法把数据行存放在两个不同的地方，所以一个表只能有一个聚簇索引。
...
```
细分又变成了 **SQL优化**（避免不使用索引）
1. 尽量避免使用in 和not in  （会导致数据库引擎放弃索引进行全表扫描）
