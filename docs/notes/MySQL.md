...
索引列上不能使用表达式或函数，否则无法使用索引。
比如索引列不要参与计算，or的两侧要么都索引列，要么都不是索引列，模糊匹配的时候%不要在头部等等

### 2. 多列索引

使用多个列作为条件进行查询时，使用多列索引比使用多个单列索引性能更好
...
### 5. 覆盖索引（https://blog.csdn.net/jh993627471/article/details/79421363）
