...
public int[][] reconstructQueue(int[][] people) {
    ...
    Arrays.sort(people, (a, b) -> (a[0] == b[0] ? a[1] - b[1] : b[0] - a[0]));  // ****表达式
    List<int[]> queue = new ArrayList<>();
    for (int[] p : people) {
        queue.add(p[1], p);                                            // add(int index,E e) 
    }
...
public int maxSubArray(int[] nums) {
    ...
    for (int i = 1; i < nums.length; i++) {
        preSum = preSum > 0 ? preSum + nums[i] : nums[i]; //前面总和大于零 就取 下个元素相加和，否则前面总和从 下个元素开始
        maxSum = Math.max(maxSum, preSum);  //  如果总和取了是 和下个元素相加的和 B ，如果相加少了则最大和仍然取之前的，如果之前总和取了下个元素变小，怎最大总和取之前的总和
