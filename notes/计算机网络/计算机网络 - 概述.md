...
* * <!-- GFM-TOC -->
      ...
      传输层: TCP UDP
      网络层: (典型设备:路由器，防火墙、多层交换机)    IP ARP ICMP
## 1. 五层协议
...
TCP   是面向连接的，经过三次“对话”才能建立起来  是可靠的，有序的，因为保证可靠性有序性速度较慢

UDP 是 非连接的协议 ，可能丢失数据 不可靠，不是有序的，速度较快

## 三次握手和四次挥手

三次握手  

第一次握手：建立连接是，客服端发生SYN包到服务器，并进入SYN_send状态

第二次握手：服务器收到SYN包，同时自己也发送一个SYN包，此时服务器进入SYN_RECV状态

第三次握手：客户端收到服务器的包，向服务器发生确认包 ACK ，客户端和服务端进入established状态，完成三次握手

四次挥手（终止连接）

第一次挥手：客户端发送一个FIN,用来关闭客户端到服务端的数据连接，客户端进入FIN_WAIT状态

第二次挥手：服务器收到FIN后，发送一个ACK给客户端，服务器进入CLOSE_WAIT状态

第三次挥手：服务器发送一个FN,进入LAST_ACK状态

第四次挥手：客户端收到FIN后进入TIME_WAIT状态，服务器进入CLOSED状态，完成四次挥手
